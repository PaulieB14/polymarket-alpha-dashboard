...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (globalData) console.log('Global data:', globalData);
    if (tradersData) console.log('Traders data:', tradersData);
    if (tradesData) console.log('Trades data:', tradesData);
  }, [globalData, tradersData, tradesData]);

  // Demo data for visualization (mixed with real data when available)
  const orderFlowData = [
  ...
  const smartMoneyData = tradersData?.accounts?.map(trader => ({
    ...trader,
    winRate: calculateWinRate(trader.scaledProfit, trader.numTrades)
  })) || [];

  const whaleActivity = tradesData?.transactions?.slice(0, 4).map(tx => ({
    timestamp: new Date(parseInt(tx.timestamp) * 1000).toLocaleString(),
    address: formatAddress(tx.user?.id),
    action: tx.type || 'TRADE',
  ...
  // Show errors if any
  if (globalError || tradersError || tradesError) {
    console.error('GraphQL Errors:', { globalError, tradersError, tradesError });
  }
